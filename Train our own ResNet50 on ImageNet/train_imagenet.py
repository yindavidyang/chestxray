...
def get_data(path, size, bs, workers):
    ...
    train = ImageList.from_folder(path / 'train')
    valid = ImageList.from_folder(path / 'valid')
    lls = ItemLists(path, train, valid).label_from_folder().transform(
...
def main( gpu:Param("GPU to run on", str)=None ):
    ...
    path = Path('.')
    tot_epochs,size,bs,lr = 60,224,512,3e-1
